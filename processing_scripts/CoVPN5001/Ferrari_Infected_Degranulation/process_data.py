...
# Date: 10-30-2024
# Purpose:  - Process CoVPN5001 Infected Degranulation data from Ferrari lab
## ---------------------------------------------------------------------------##
import pandas as pd
import datetime, os
import numpy as np
...
def main():
    infected_degran_analysis_path = "/trials/covpn/p5001/s001/qdata/LabData/Ferrari_pass-through/Ferrari_CoVPN5001_Infected Cell Degranulation_Analysis_2022-03-23.csv"
    infected_degran_metadata_path = "/trials/covpn/p5001/s001/qdata/LabData/Ferrari_pass-through/Ferrari_CoVPN5001_Infected Cell Degranultaion_Metadata_2022-03-25.csv"

    data = pd.read_csv(infected_degran_analysis_path)
    metadata = pd.read_csv(infected_degran_metadata_path)
    ...
    ldms = pd.read_csv(
        constants.LDMS_PATH_HVTN, #this is correct, idk why this is saved here
    ...
    metadata_dict = {
        ...
        'assay_subtype': 'Infected Cell CD107a Degranulation',
        'assay_details': 'Infected',
        'specrole': 'Sample',
    ...
    outputs = sdmc.standard_processing(
        input_data=data,
        input_data_path=infected_degran_analysis_path,
        guspec_col='guspec',
        network='covpn',
        metadata_dict=metadata_dict,
        ldms=ldms,
        additional_input_paths={'input_metadata_file_name': infected_degran_metadata_path}
    )
    ...
    discrepancy = np.abs(check - outputs['result_background_subtracted_pct_cd107a+_nk_cells']).max()
    if discrepancy > 1e-8:
    ...
    summary.to_excel(savedir + "CoVPN5001_Infected_degran_data_summary.xlsx")
